//
//  ToggleCheckbox.swift
//  CoreFramework
//
//  Created by Daniel Moura on 06/11/25.
//

import Foundation
import UIKit

public class ToggleCheckbox: UIButton {
  private var isChecked: Bool = false
  private let checkedImage: UIImage = UIImage(systemName: "checkmark.square.fill")!
  private let uncheckedImage: UIImage = UIImage(systemName: "square")!
  
  public init() {
    super.init(frame: .zero)
    self.tintColor = Colors.primaryBlueBase
    self.setImage(uncheckedImage, for: .normal)
    self.addTarget(self, action: #selector(toggle), for: .touchUpInside)
  }
  
  required init?(coder: NSCoder) {
    fatalError("init(coder:) has not been implemented")
  }
  
  @objc
  private func toggle() {
    isChecked.toggle()
    self.setImage(isChecked ? checkedImage : uncheckedImage, for: .normal)
  }
  
  public func getIsCheckedState() -> Bool {
    return isChecked
  }
}

